...
public:
  ...
  std::pair<std::vector<MatPoly>, uint16_t> Encode(
    ...
    uint16_t counter = 0;
    while (counter < MAX_ATTEMPTS_NUM) {
      ++counter;
      try {
        ...
        size_t randint = std::mt19937_64(counter)();
    ...
    if (counter >= MAX_ATTEMPTS_NUM) {
      // std::cerr << "OCD encode: Maximum attempts reached without success." << std::endl;
      throw std::runtime_error("Maximum attempts reached without success");
    }
    return {};
  }

  std::vector<MatPoly> Decode(std::vector<MatPoly> cts_hat, uint16_t counter) {
    size_t m = ceil(code.r * (double)code.k);
