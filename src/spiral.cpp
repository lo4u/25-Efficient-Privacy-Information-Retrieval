...
void gen_deterministic_random_pt(MatPoly &M, unsigned seed) {
    assert(!M.isNTT);
    // 设置随机数生成器的种子
    // unsigned seed = 39; // 使用一个固定的种子值
    std::mt19937 gen(seed); // 创建一个基于Mersenne Twister算法的生成器

    std::uniform_int_distribution<> dis(0, p_db - 1);

    for (size_t i = 0; i < M.rows * M.cols * poly_len; i++) {
        M.data[i] = dis(gen) % (p_db);
    }
}
